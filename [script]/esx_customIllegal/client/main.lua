...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('esx_customIllegal:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_customIllegal:installMod')
AddEventHandler('esx_customIllegal:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_customIllegal:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_customIllegal:cancelInstallMod')
AddEventHandler('esx_customIllegal:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_customIllegal:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('esx_customIllegal:buyMod', price)
		...
		menu.close()
		TriggerEvent('esx_customIllegal:cancelInstallMod')
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'illegalmecano') or not Config.IsillegalmecanoJobOnly then
				for k,v in pairs(Config.Zones) do
